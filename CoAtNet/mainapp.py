...
from spec111 import TIFImageFolder
from utils import train_one_epoch, evaluate
from modelapp import coatnet_1 as create_model
...
def collate_fn(batch):
    ...
    transformed_images = []
    for image in images:
        transformed_images.append(transforms.Resize((224, 224))(image))
...
def run_once(args, run_id):
    ...
    logger = get_logger(
        f'/mnt/data/home/wangjialuo/code/coatnet/coatoapp_run{run_id}.log'
    )
    ...
    for epoch in range(args.epochs):
        ...
        if f1_va > best_f1:
            ...
            torch.save(
                model.state_dict(),
                f"./weights/coatoappbest_run{run_id}.pth"
            )
    ...
    plt.legend()
    plt.savefig(f"coatoapploss_run{run_id}.png")
    plt.close()
    ...
    if best_matrix is not None:
        ...
        plt.tight_layout()
        plt.savefig(f"coatoapp_cm_run{run_id}.png")
        plt.close()

    return best_metrics  # <-- 返回每 run 的最佳 F1 epoch 对应指标
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lrf', type=float, default=0.0005)
    ...
    # ===================== 写入最后一轮 log =====================
    final_logger = get_logger(f'/mnt/data/home/wangjialuo/code/coatnet/coatoapp_run3.log')
    final_logger.info('========== Final Average Results (3 runs, best F1 epoch per run) ==========')
    final_logger.info(
        f"Accuracy : {mean_metrics[0]:.4f}, Precision: {mean_metrics[1]:.4f}, "
