    ...
    def forward(ctx, input, weight, kernel_size, stride, padding, dilation):
        ...
        with torch.cuda.device_of(input):
            f = load_kernel(
                ...
                weight_heads=int(weight_heads),
                weight_channels=int(weight_channels),
                bottom_height=int(input_height),
                bottom_width=int(input_width),
                top_height=int(output_height),
                top_width=int(output_width),
    ...
    def backward(ctx, grad_output):
        ...
        with torch.cuda.device_of(input):
            ...
            if ctx.needs_input_grad[1]:
                ...
                n = int(grad_weight.numel() // weight.shape[3])
                opt['nthreads'] = n
