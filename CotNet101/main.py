...
from model import cotnet101 as create_model
...
def run_once(args, run_id):
    ...
    logger = get_logger(
        f'/mnt/data/home/wangjialuo/code/CotNet101/cototif_run{run_id}.log'
    )
    ...
    for epoch in range(args.epochs):
        ...
        if f1_va > best_f1:
            ...
            torch.save(
                model.state_dict(),
                f"./weights/cototifbest_run{run_id}.pth"
            )
    ...
    plt.legend()
    plt.savefig(f"cototifloss_run{run_id}.png")
    plt.close()
    ...
    if best_matrix is not None:
        ...
        plt.tight_layout()
        plt.savefig(f"cototif_cm_run{run_id}.png")
        plt.close()
