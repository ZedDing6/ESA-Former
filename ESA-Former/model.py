...
class SpectralConv1D(nn.Module):
    def __init__(self, in_bands: int = 5, out_channels: int = 32, spatial_size: int = 24):
        super().__init__()
...
class SpectralTokenization(nn.Module):
    def __init__(self, img_size: int = 200, in_c: int = 4, embed_dim: int = 768):
        super().__init__()
        ...
        self.token_h = (img_size + self.padding_h * 2 - self.kernel_h) // self.stride_h + 1
        self.token_w = (img_size + self.padding_w * 2 - self.kernel_w) // self.stride_w + 1
        self.pos_embed = nn.Parameter(torch.zeros(1, self.token_h * self.token_w, embed_dim))
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.attn = SpectralAttention(embed_dim, dct_h=self.token_h, dct_w=self.token_w)
...
class TokenMerge(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, kernel_size: int = 2, stride: int = 2, padding: int = 0):
        super().__init__()
    ...
    def sim_calib(self, x: torch.Tensor) -> torch.Tensor:
        B, C, H, W = x.shape
    ...
    def forward(self, x: torch.Tensor, H: int, W: int):
        ...
        cls = cls_token + 0.5 * tokens.mean(dim=1, keepdim=True)
        ...
        cls_token = self.cls_proj(cls)
        x_out = torch.cat([cls_token, x_merged_flat], dim=1)
        return x_out, H_new, W_new


class VisionTransformer(nn.Module):
    def __init__(self, img_size: int = 200, in_c: int = 5, num_classes: int = 1000,
        ...
        self.patch_embed = SpectralTokenization(
            img_size=img_size, in_c=in_c, embed_dim=192
        )
        self.dist_token = nn.Parameter(torch.zeros(1, 1, embed_dim)) if distilled else None
        self.pos_drop = nn.Dropout(p=drop_ratio)

        # 使用drop_path_ratio创建dpr列表（如果depth > 0）
        # 注意：这里depth参数未直接使用，因为是分阶段定义的
        dpr = [x.item() for x in torch.linspace(0, drop_path_ratio, 3)]  # 3个阶段

        self.stage1_blocks = nn.Sequential(*[
            Block(dim=192, num_heads=4, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        self.stage2_blocks = nn.Sequential(*[
            Block(dim=384, num_heads=8, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        self.stage3_blocks = nn.Sequential(*[
            Block(dim=768, num_heads=12, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        if representation_size and not distilled:
            ...
            self.pre_logits = nn.Sequential(OrderedDict([
                ("fc", nn.Linear(768, representation_size)),
                ("act", nn.Tanh())
            ]))
        else:
        ...
        self.head_dist = nn.Linear(768, num_classes) if distilled and num_classes > 0 else None
    ...
    def forward_features(self, x: torch.Tensor):
        ...
        # 如果存在dist_token，在这里添加
        if self.dist_token is not None:
            cls_tokens = x[:, :1, :]
            dist_token = self.dist_token.expand(x.shape[0], -1, -1)
            x = torch.cat((cls_tokens, dist_token, x[:, 1:, :]), dim=1)
    ...
    def forward(self, x: torch.Tensor, spectral_x: torch.Tensor):
...
def vit_base_patch16_224_in21k(num_classes: int = 21843, has_logits: bool = True) -> VisionTransformer:
    model = VisionTransformer(img_size=200,
                              embed_dim=768,
                              depth=12,
                              num_heads=6,
