...
from modelapp import vit_base_patch16_224_in21k as create_model
...
def run_once(args, run_id):
    ...
    logger = get_logger(
        f'/mnt/data/home/wangjialuo/code/Vision-Trans/ablat/esaformer_app_run{run_id}.log'
    )
    ...
    for epoch in range(args.epochs):
        ...
        if f1_va > best_f1:
            ...
            torch.save(
                model.state_dict(),
                f"./weights/esaformer_appbest_run{run_id}.pth"
            )
    ...
    plt.legend()
    plt.savefig(f"esaformer_apploss_run{run_id}.png")
    plt.close()
    ...
    if best_matrix is not None:
        ...
        plt.tight_layout()
        plt.savefig(f"esaformer_app_cm_run{run_id}.png")
        plt.close()
