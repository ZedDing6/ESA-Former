...
class Conv2Former(nn.Module):
    def __init__(self, kernel_size, img_size=224, in_chans=5, num_classes=1000, depths=[3, 3, 9, 3],
                 dims=[96, 192, 384, 768], window_sizes=[14, 14, 14, 7],
        ...
        for i in range(len(dims)):
            stage = nn.Sequential(
                *[Block(index=cur + j, dim=dims[i], kernel_size=kernel_size, drop_path=dp_rates[cur + j],
                        num_head=num_heads[i], window_size=window_sizes[i], mlp_ratio=mlp_ratios[i]) for j in
                  range(depths[i])]
            )
...
def conv2former_t(pretrained=False, **kwargs):
    model = Conv2Former(kernel_size=11, dims=[72, 144, 288, 576], mlp_ratios=[4, 4, 4, 4], depths=[3, 3, 12, 3],
                        **kwargs)
    model.default_cfg = _cfg()
    return model
...
def conv2former_s(pretrained=False, **kwargs):
    model = Conv2Former(kernel_size=11, dims=[72, 144, 288, 576], mlp_ratios=[4, 4, 4, 4], depths=[4, 4, 32, 4],
                        **kwargs)
    model.default_cfg = _cfg()
    return model
...
def conv2former_b(pretrained=False, **kwargs):
    model = Conv2Former(kernel_size=11, dims=[96, 192, 384, 768], mlp_ratios=[4, 4, 4, 4], depths=[4, 4, 34, 4],
                        **kwargs)
    model.default_cfg = _cfg()
    return model
...
def conv2former_b_22k(pretrained=False, **kwargs):
    model = Conv2Former(kernel_size=7, dims=[96, 192, 384, 768], mlp_ratios=[4, 4, 4, 4], depths=[4, 4, 34, 4],
                        **kwargs)
    model.default_cfg = _cfg()
    return model
...
def conv2former_l(pretrained=False, **kwargs):
    model = Conv2Former(kernel_size=11, dims=[128, 256, 512, 1024], mlp_ratios=[4, 4, 4, 4], depths=[4, 4, 48, 4],
                        **kwargs)
    model.default_cfg = _cfg()
    return model