...
from modelapp import conv2former_b as create_model
...
def run_once(args, run_id):
    ...
    logger = get_logger(
        f'/mnt/data/home/wangjialuo/code/conv2former/cv2foapp_run{run_id}.log'
    )
    ...
    for epoch in range(args.epochs):
        ...
        if f1_va > best_f1:
            ...
            torch.save(
                model.state_dict(),
                f"./weights/cv2foappbest_run{run_id}.pth"
            )
    ...
    plt.legend()
    plt.savefig(f"cv2foapploss_run{run_id}.png")
    plt.close()
    ...
    if best_matrix is not None:
        ...
        plt.tight_layout()
        plt.savefig(f"cv2foapp_cm_run{run_id}.png")
        plt.close()
