...
from omodel import swin_tiny_patch4_window7_224 as create_model
...
def main(args):
    ...
    save_path = './{}Net.pth'.format("Swin")
    logger=get_logger('/home/wangjialuo/pyproject/swin/oSwin.log')
    train_losses=[]
    val_losses=[]
    best_acc = 0
    best_Matrix=None
    ...
    plt.tight_layout()
    plt.savefig('oSwin.png')  # 保存混淆矩阵图像
    plt.show()
    plt.figure()
    ...
    plt.savefig('oSwinloss.png')  # 保存loss图为图片文件
    plt.show()  # 显示loss图
