...
from modelapp import swin_base_patch4_window7_224 as create_model
...
def run_once(args, run_id):
    ...
    logger = get_logger(
        f'/mnt/data/home/wangjialuo/code/swin/swinoapp_run{run_id}.log'
    )
    ...
    for epoch in range(args.epochs):
        ...
        if f1_va > best_f1:
            ...
            torch.save(
                model.state_dict(),
                f"./weights/swinoappbest_run{run_id}.pth"
            )
    ...
    plt.legend()
    plt.savefig(f"swinoapploss_run{run_id}.png")
    plt.close()
    ...
    if best_matrix is not None:
        ...
        plt.tight_layout()
        plt.savefig(f"swinoapp_cm_run{run_id}.png")
        plt.close()
