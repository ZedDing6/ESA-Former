...
def run_once(args, run_id):
    ...
    logger = get_logger(
        f'/mnt/data/home/wangjialuo/code/swin/swinotif_run{run_id}.log'
    )
    ...
    for epoch in range(args.epochs):
        ...
        if f1_va > best_f1:
            ...
            torch.save(
                model.state_dict(),
                f"./weights/swinotifbest_run{run_id}.pth"
            )
    ...
    plt.legend()
    plt.savefig(f"swinotifloss_run{run_id}.png")
    plt.close()
    ...
    if best_matrix is not None:
        ...
        plt.tight_layout()
        plt.savefig(f"swinotif_cm_run{run_id}.png")
        plt.close()
