...
class EfficientNet(nn.Module):#从模型定义进来先进入到这里
    def __init__(self,
                 width_coefficient: float,
        ...
        #定义列表配置和构建模块，这些是模块参数，方便模块的初始化和配置
        ...
        b = 0
        num_blocks = float(sum(round_repeats(i[-1]) for i in default_cnf))#这个计算是在构建EfficientNet模型时非常重要的一步，因为它确定了模型中Inverted Residual块的数量，从而影响了整个网络的深度和复杂度。通过调整default_cnf中每个子列表的重复次数，可以灵活地控制EfficientNet模型的深度，并根据具体任务的需求进行调整。
        ...
        # first conv
        layers.update({"stem_conv": ConvBNActivation(in_planes=5,
