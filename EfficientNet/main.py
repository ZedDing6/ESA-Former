...
def run_once(run_id, args):
    ...
    logger = get_logger(f'/mnt/data/home/wangjialuo/code/Efficient/effotif_run{run_id}.log')
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > best_val_accuracy:
            ...
            torch.save(model.state_dict(), f"./weights/effotifmodel_run{run_id}.pth")
    ...
    if best_confusion_matrix is not None:
        ...
        plt.tight_layout()
        plt.savefig(f'effotif_cm_run{run_id}.png')
        plt.close()
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(f'effotif_loss_run{run_id}.png')
    plt.close()
