...
from model import moganet_base
...
def run_once(run_id, args):
    ...
    logger = get_logger(f'/mnt/data/home/wangjialuo/code/moganet/tifmoga_run{run_id}.log')
    ...
    flops, params = count_flops_and_params(
        net, img_size=200, in_channels=5, device="cuda"
    )
    ...
    save_path = f'./weights/tifmoga_run{run_id}.pth'
    ...
    plt.tight_layout()
    plt.savefig(f'tifmoga_cm_run{run_id}.png')
    plt.close()
    ...
    plt.legend()
    plt.savefig(f'tifmoga_loss_run{run_id}.png')
    plt.close()
...
if __name__ == '__main__':
    ...
    final_logger = get_logger(
        '/mnt/data/home/wangjialuo/code/moganet/tifmoga_run3.log'
    )
